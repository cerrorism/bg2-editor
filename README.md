...
**Not yet implemented:**
- The rest of the original request list is in place — attributes, inventory, spellbook, name resolution (including the picker), and now item comparison stats. Possible next steps: verifying an edited save actually loads in-game now that the corruption bug is fixed, and BG2:EE-specific validation (everything so far has been tested against BG1:EE, since that's the save data available).
